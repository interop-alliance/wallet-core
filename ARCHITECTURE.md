...
the server. It is hard-coded once: `identity/agents.ts`'s
`zcapClientForSigner({ signer })` is the library's one `ZcapClient` construction
site, and the wrappers that sign under a different key id (`webvh/zcap.ts`'s
two, `clientAnnex/zcap.ts`'s ladder VM) build the signer and hand it there.
