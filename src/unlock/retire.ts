 ...
 * healthy.
 *
 * No stage runs before the edit, and no sibling credential's record is
 * touched anywhere in the ceremony. Every unlock record's frame proof is
 * signed by its own credential's unlock identity key, and its bridge and
 * `delegatedClients` sibling delegations by that credential's own ladder VM
 * (`decisions/0019`), so the only record this strike can rot is the retired
 * credential's own -- which dies with the unlock Space the app deletes as
 * part of the change-method ceremony. A sibling credential's bridge is
 * refreshed by that credential's own login, on the three staleness axes
 * (expiry, the renewal window, a signer that left the document).
 *
 ...
 * runs.
 *
 * The edit carries the retirement gate (`decisions/0015`): a credential
 ...
 * `UnclaimedLadderVmRetirementError`, inside the edit before its entry
 ...
 * was read from, and the inventory edit's ladder VM report (`ladderVm`: what the edit struck, and
